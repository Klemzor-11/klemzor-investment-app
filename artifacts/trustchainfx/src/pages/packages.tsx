...
const packages = [
  ...
  },
  {
    id: "growth",
    name: "Growth",
    description: "Our core offering. Optimized HFT strategies for balanced risk/reward.",
    min: 200,
    roi: 14,
    lock: 60,
  ...
  const handleInvest = (pkgName: string) => {
    if (!user) {
      toast({
        ...
        description: "Please login to deploy capital.",
        variant: "destructive"
      });
      setLocation("/login");
      return;
    ...
    toast({
      title: "Capital Deployment Initiated",
      description: `Your ${pkgName} package investment is processing.`,
    ...
    <Layout>
      <div className="container mx-auto px-4 py-16 max-w-7xl">
        <div className="text-center mb-16 max-w-2xl mx-auto">
          <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-4">Investment Tiers</h1>
                ...
                <CardHeader>
                  ...
                  <div className="mt-4 pb-4 border-b border-border/50">
                    <div className="text-4xl font-mono font-bold text-foreground">
                      <span className="text-xl text-muted-foreground font-sans">From </span>
                      ${pkg.min}
                    </div>
                    <div className="flex items-center gap-4 mt-2">
                      <div className="text-sm font-medium text-primary">
                        {pkg.roi}% <span className="text-muted-foreground font-normal">Monthly ROI</span>
                      </div>
                      <div className="text-sm text-muted-foreground flex items-center">
                        <ShieldAlert className="w-3 h-3 mr-1" /> {pkg.lock} Days
                      </div>
                    </div>
                  </div>
                </CardHeader>
